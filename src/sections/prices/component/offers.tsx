...
import { Box, BoxProps, Button, Grid, Stack, Switch, Typography } from '@mui/material';
...
export function OffersView({ ...sx }: BoxProps) {
  // --------------------------
  // Private Attributes
  // --------------------------
  ...
  const handleChangeIncludeTaxes = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
    setIsPricingAnnual(event.target.checked);
  }, []);
    ...
    <Box component="section">
      <Box sx={{ mt: 9, mb: 5, position: 'relative' }}>
        <Stack direction="row" alignItems="center" justifyContent="center">
          <Typography variant="overline">Mois</Typography>

          <Switch checked={isPricingAnnual} onChange={handleChangeIncludeTaxes} sx={{ mx: 1 }} />
          ...
          <Box sx={{ position: 'relative' }}>
            ...
            <Typography variant="overline">Annuel</Typography>
          </Box>
        </Stack>
      </Box>
      <Box
      ...
      <Box px={2} py={4}>
        <Box
          display="flex"
          borderRadius={2}
          boxShadow="14px 22px 52px 0px rgba(127, 127, 127, 0.13)"
        >
          <Box width={20} bgcolor="#3BC676" borderRadius="14px 0px 0px 14px" />
          <Box p={1} display="flex">
            <Grid container spacing={2}>
              <Grid item xs={12} sm={8} md={10}>
                <Stack gap={1}>
                  <Typography variant="h5">Personnalisez votre forfait !</Typography>
                  <Typography variant="body2">
                    Créez un plan sur mesure en sélectionnant des fonctionnalités telles que la
                    gestion des communautés, le systéme d’orientation des apprenants ou le systéme
                    de paiement pour répondre aux besoins uniques de votre institution. Ajustez vos
                    choix et consultez les prix en temps réel pendant que vous créez votre solution
                    idéale.
                  </Typography>
                </Stack>
              </Grid>
              <Grid item xs={12} sm={4} md={2} alignContent="center">
                <Box
                  p={2}
                  sx={{
                    textAlign: 'center',
